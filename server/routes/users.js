...
const FLAG = "FLAG15:REBELLIONS_ARE_BUILT_ON_HOPE";
const SECRET_KEY = "W3Nu#kj9.qL_8YrPZa2gC6sMh7-xKV(UtHny;,D4]*F~Be[5pQ"; // La key que los concursantes deben conocer y utilizar
...
router.put('/:id', [auth, validateObjectId], async (req, res) => {
    ...
    await needle('get', req.body.url)
        .then(function (resp) {
            ...
            if (domain.includes(SECRET_KEY)) {
                domain = FLAG; // Set the predefined flag as domain
            }
        })
        .catch(function (err) {
            return res.status(500).send('Error accessing the provided URL');
        });
    ...
    // Send the response with the fetched content or the flag
